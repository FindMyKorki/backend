...
async def accept_booking(booking_id: int, _user_response=Depends(authenticate_user)):
...
async def reject_booking(booking_id: int, _user_response=Depends(authenticate_user)):
...
async def cancel_booking(booking_id: int, _user_response=Depends(authenticate_user)):
...
async def mark_booking_as_paid(booking_id: int, _user_response=Depends(authenticate_user)):
...
async def mark_booking_as_unpaid(booking_id: int, _user_response=Depends(authenticate_user)):
