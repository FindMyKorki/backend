...
async def subtract_time_blocks_test(
    ...
    for unavail in unavailabilities:
        ...
        if start and end:
            start = standardize_datetime(datetime.fromisoformat(start.replace('Z', '+00:00')) if isinstance(start, str) else start)
            end = standardize_datetime(datetime.fromisoformat(end.replace('Z', '+00:00')) if isinstance(end, str) else end)
